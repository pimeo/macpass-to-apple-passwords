...
const copy_or_move = (action, pattern, base_dir, dest_dir, options) => {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    for (let i = 0; i < from_filepaths.length; i++) {
      ...
      files.push(`Action ${action}: ${filepath} to ${path.join(to_filepath, parsed.base)}`)
      await fs[`${action}`](filepath, path.join(to_filepath, parsed.base), options)
    }
...
const delete_or_remove = (pattern, base_dir, options) => {
  return new Promise( async (resolve, reject) => {
...
module.exports.copy = (pattern = null, base_dir = null, dest_dir = null, options = {}) => {
  return copy_or_move('copy', pattern, base_dir, dest_dir, options)
}

module.exports.move = (pattern = null, base_dir = null, dest_dir = null, options = {}) => {
  return copy_or_move('move', pattern, base_dir, dest_dir, options)
}

module.exports.rename = (pattern = null, base_dir = null, dest_dir = null, options = {}) => {
  return rename(pattern, base_dir, dest_dir, options)
}

module.exports.delete = (pattern = [], base_dir = null, options = {}) => {
  return delete_or_remove(pattern, base_dir, options)
}
